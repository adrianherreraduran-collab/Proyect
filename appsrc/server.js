...
function optionalAuth(req,res,next){const h=String(req.headers.authorization||''),t=h.startsWith('Bearer ')?h.slice(7):'';if(!t){req.user=null;return next()}try{req.user=jwt.verify(t,JWT_SECRET);next()}catch{return res.status(401).json({error:'Sesión no válida'})}}
...
function publicOrder(o={}){
  ...
  const paymentMethod=String(safe.paymentMethod||'').toLowerCase()==='stripe'?'Tarjeta (Stripe)':(safe.paymentMethod||'');
...
async function createStripePaymentLink(req,d,o,accessToken=''){
  ...
  if(o.stripeSessionUrl&&Number(o.stripeSessionExpiresAt||0)>Date.now())return {id:String(o.stripeSessionId||''),url:o.stripeSessionUrl,reused:true};
  ...
  const checkout=await stripe.checkout.sessions.create({mode:'payment',payment_method_types:['card'],line_items,success_url:success.toString().replace('%7BCHECKOUT_SESSION_ID%7D','{CHECKOUT_SESSION_ID}'),cancel_url:cancel.toString(),client_reference_id:o.id,customer_email:o.customer?.email||undefined,billing_address_collection:'required',phone_number_collection:{enabled:true},locale:'es',metadata:{source:'FVMarket',orderId:o.id,orderNumber:o.number,quoteId:String(o.quoteId||''),fulfillment_model:'sin_stock_fisico',delivery_mode:'normal',transport_amount:String(o.delivery||0)}});
...
async function confirmStripePayment(req,session){
  ...
  const firstConfirmation=!o.paidAt;
  ...
  save(d);
  if(firstConfirmation){scheduleOrderEmail(req,o.id,'order_confirmation');if(invoice)scheduleOrderEmail(req,o.id,'invoice_issued')}
...
function legalPage(section='condiciones',settings={}){
  ...
  return `<!doctype html><html lang="es"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>${page.title} · FVMarket</title><style>body{margin:0;background:#f2f6f8;color:#12263f;font:16px/1.65 Arial,sans-serif}header{background:#06345f;color:#fff;padding:22px}header a{color:#fff;text-decoration:none;font-size:28px;font-weight:900}header span{color:#82c341}main{max-width:840px;margin:28px auto;background:#fff;padding:38px;border-radius:14px;box-shadow:0 10px 35px #06345f18}h1{color:#06345f;line-height:1.2}h2{color:#06345f;font-size:19px;margin-top:28px}a{color:#075b93}nav{display:flex;gap:14px;flex-wrap:wrap;margin-bottom:26px}nav a{font-weight:700}.updated{color:#64748b;font-size:13px}@media(max-width:700px){main{margin:0;padding:24px;border-radius:0}}</style></head><body><header><a href="/">FV<span>Market</span></a></header><main><nav><a href="/legal/condiciones">Condiciones</a><a href="/legal/privacidad">Privacidad</a><a href="/legal/devoluciones">Devoluciones</a></nav><h1>${page.title}</h1><p class="updated">Última actualización: 26 de septiembre de 2026</p>${page.body}</main></body></html>`;
}
app.get('/legal/:section?',(req,res)=>{const section=String(req.params.section||'condiciones').toLowerCase();if(!['condiciones','privacidad','devoluciones'].includes(section))return res.status(404).send('Página no encontrada');res.type('html').send(legalPage(section,read().settings||{}))});
...
app.post('/api/quotes/:id/payment-link',auth,requireCustomerReady,async(req,res)=>{
  ...
  try{
    ...
    const link=await createStripePaymentLink(req,d,o);q.orderId=o.id;q.stripeSessionId=link.id;q.stripeSessionUrl=link.url;q.paymentLinkCreatedAt=q.paymentLinkCreatedAt||new Date().toISOString();save(d);if(created)scheduleOrderEmail(req,o.id,'payment_link_created',{paymentLinkUrl:link.url});res.json({url:link.url,orderNumber:o.number,reused:!!link.reused});
...
app.post('/api/orders',(req,res)=>res.status(405).json({error:'FVMarket acepta pedidos online únicamente mediante pago seguro con tarjeta en Stripe.'}));
app.post('/api/checkout/stripe',optionalAuth,async(req,res)=>{
  if(!stripe)return res.status(503).json({error:'El pago con tarjeta no está configurado todavía'});
  ...
  catch(e){d.orders=d.orders.filter(x=>x.id!==order.id);save(d);console.error('Stripe checkout:',e.message);res.status(502).json({error:`No se pudo iniciar el pago con tarjeta: ${e.message}`})}
});
...
app.get('/api/admin/orders',ordersManager,(req,res)=>res.json(read().orders.sort((a,b)=>b.createdAt.localeCompare(a.createdAt)).map(adminOrder)));
