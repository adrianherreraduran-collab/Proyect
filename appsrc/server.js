...
function read(){try{return JSON.parse(fs.readFileSync(DATA_FILE,'utf8'))}catch(e){const d=seed();save(d);return d}}
...
app.post('/api/auth/register',async(req,res)=>{const {name,email,password}=req.body||{};if(!name||!email||!password||password.length<6)return res.status(400).json({error:'Nombre, email y contraseña de al menos 6 caracteres son obligatorios'});const d=read();if(d.users.some(u=>u.email.toLowerCase()===email.toLowerCase()))return res.status(409).json({error:'Ese email ya está registrado'});const u={id:id('usr'),name:String(name).trim(),email:String(email).trim().toLowerCase(),password:await bcrypt.hash(password,12),role:d.users.length===0?'admin':'customer',createdAt:new Date().toISOString()};d.users.push(u);save(d);res.json({token:token(u),user:safeUser(u)});});
app.post('/api/auth/login',async(req,res)=>{const {email,password}=req.body||{};const d=read();const u=d.users.find(x=>x.email.toLowerCase()===String(email||'').toLowerCase());if(!u||!(await bcrypt.compare(password||'',u.password)))return res.status(401).json({error:'Email o contraseña incorrectos'});res.json({token:token(u),user:safeUser(u)});});
...
app.post('/api/admin/import-url',admin,async(req,res)=>{const url=String(req.body.url||'').trim();if(isUnsafeUrl(url))return res.status(400).json({error:'URL no permitida'});try{const r=await axios.get(url,{timeout:8000,maxContentLength:1200000,headers:{'User-Agent':'FVMarket/1.0'}});const $=cheerio.load(r.data);const title=($('meta[property="og:title"]').attr('content')||$('h1').first().text()||$('title').text()||'').trim().slice(0,180);const image=($('meta[property="og:image"]').attr('content')||$('img').first().attr('src')||'').trim();const description=($('meta[property="og:description"]').attr('content')||$('meta[name="description"]').attr('content')||'').trim().slice(0,800);const text=$('body').text().replace(/\s+/g,' ');const m=text.match(/(?:€\s*([0-9]+(?:[.,][0-9]{1,2})?)|([0-9]+(?:[.,][0-9]{1,2})?)\s*€)/);const sourcePrice=m?Number((m[1]||m[2]).replace(',','.')):0;const margin=Number(req.body.margin||40);const price=sourcePrice?+(sourcePrice*(1+margin/100)).toFixed(2):0;res.json({title,image,description,sourceUrl:url,sourcePrice,margin,price,category:'Otros',ref:'FVM-'+Date.now().toString().slice(-6),stock:'bajo_pedido',published:false,featured:false})}catch(e){res.status(422).json({error:'No se pudo leer esa URL. Puedes crear el producto manualmente con la URL como referencia.'})}});
