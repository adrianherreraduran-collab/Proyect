...
app.post('/api/stripe/webhook',express.raw({type:'application/json'}),async(req,res)=>{
  ...
  if(event.type==='checkout.session.completed'||event.type==='checkout.session.async_payment_succeeded'){
    ...
    if(s.payment_status==='paid'||event.type.endsWith('succeeded')){
      ...
      if(o){o.status=paidOrderStatus(o.status)?o.status:'pagado';o.paidAt=o.paidAt||new Date().toISOString();o.paymentIntentId=String(typeof s.payment_intent==='string'?s.payment_intent:s.payment_intent?.id||'');o.stripePaymentStatus=String(s.payment_status||'paid');operations.recordPaymentConfirmation(d,o,{id:'stripe_webhook',name:'Pago online',role:'system'});if(o.quoteId){const q=(d.quotes||[]).find(x=>x.id===o.quoteId);if(q){q.status='pagado';q.paidAt=q.paidAt||o.paidAt}}const invoice=issueInvoiceForOrder(d,o);if(invoice)operations.recordInvoiceIssued(d,o,invoice,{id:'stripe_webhook',name:'Pago online',role:'system'});save(d);scheduleOrderEmail(req,o.id,'order_confirmation');if(invoice)scheduleOrderEmail(req,o.id,'invoice_issued')}
    }
  }
...
function ensureLogisticsSettings(d){
  ...
  if(d.settings.rutaFVOrigin==null)d.settings.rutaFVOrigin='';
  if(d.settings.rutaFVDeliveryDateMode==null)d.settings.rutaFVDeliveryDateMode='gestion_rutafv';
}
function fvmarketOrigin(d){
  const s=d.settings||{};
  return String(s.rutaFVOrigin||[s.fiscalAddress,s.fiscalPostalCode,s.fiscalCity].filter(Boolean).join(', ')||'').trim();
}
...
function profileSummary(d,u){const quotes=(d.quotes||[]).filter(q=>q.userId===u.id).sort((a,b)=>String(b.createdAt||'').localeCompare(String(a.createdAt||'')));const orders=(d.orders||[]).filter(o=>o.userId===u.id).sort((a,b)=>String(b.createdAt||'').localeCompare(String(a.createdAt||''))).map(publicOrder);const invoices=(d.invoices||[]).filter(x=>x.userId===u.id).sort((a,b)=>String(b.issuedAt||'').localeCompare(String(a.issuedAt||''))).map(publicInvoice);const payments=orders.map(o=>({id:o.id,orderNumber:o.number,amount:o.total,method:String(o.paymentMethod||'').toLowerCase()==='stripe'?'Pago online':(o.paymentMethod||'Transferencia bancaria'),status:['pagado','entregado'].includes(String(o.status||''))?'pagado':'pendiente',createdAt:o.createdAt}));return {quotes,orders,invoices,payments}}
...
app.post('/api/quotes',auth,requireCustomerReady,async(req,res)=>{
  ...
  try{const profile=req.customer||{},input=req.body?.customer||{},u={...profile,...input};const rawDestination=input.deliveryAddress||input;const a={address:String(rawDestination.address||profile.deliveryAddress?.address||'').trim(),city:String(rawDestination.city||profile.deliveryAddress?.city||'').trim(),postalCode:String(rawDestination.postalCode||profile.deliveryAddress?.postalCode||'').trim()};if(!a.address||!a.city||!a.postalCode)return res.status(400).json({error:'Completa y valida la dirección de entrega antes de crear el presupuesto.'});const supplied=req.body?.rutaFVQuote;let r=null;if(validTransportQuote(supplied,req.user.id,req.body?.items||[],a)){r=supplied}else{const payload={clientCode:d.settings.rutaFVClientCode||RUTAFV_CLIENT_CODE,customer:{name:u.name,email:u.email,phone:u.phone||''},origin:{address:fvmarketOrigin(d)},destination:a,destinationText:[a.address,a.city,a.postalCode].filter(Boolean).join(', '),items:normalized.map(x=>({id:x.productId,ref:x.ref,title:x.title,qty:x.qty,weightKg:Number(d.products.find(p=>p.id===x.productId)?.weightKg||0),supplierId:x.supplierId,sourceProvider:x.sourceProvider})),orderSource:'FVMarket',fulfillmentModel:'sin_stock_fisico',deliveryMode:'normal',express:false,requestType:'quote'};r=await rutaFVRequest(RUTAFV_QUOTE_PATH,payload)}delivery=Math.max(0,Number(r.amount||r.total||0));transport={provider:'RutaFV',amount:delivery,quoteId:String(r.id||r.quoteId||''),origin:fvmarketOrigin(d),destination:a,deliveryMode:'normal',express:false,estimatedDeliveryDate:r.estimatedDeliveryDate||r.deliveryDate||null,deliveryDateStatus:r.deliveryDateStatus||'pendiente_planificacion'}}catch(e){return res.status(503).json({error:'No se pudo calcular el transporte: '+e.message})}
...
app.post('/api/admin/orders/:id/create-rutafv-delivery',ordersManager,async(req,res)=>{try{const d=read();const o=d.orders.find(x=>x.id===req.params.id);if(!o)return res.status(404).json({error:'Pedido no encontrado'});if(!o.transport?.requested)return res.status(400).json({error:'Este pedido no tiene transporte RutaFV'});if(o.transport.deliveryId)return res.json(o);const u=d.users.find(x=>x.id===o.userId)||{};const payload={clientCode:RUTAFV_CLIENT_CODE,externalOrderId:o.id,externalOrderNumber:o.number,customer:{name:o.customer?.name||u.name||'',email:o.customer?.email||u.email||'',phone:o.customer?.phone||o.phone||''},destination:{address:o.customer?.address||o.address||'',city:o.customer?.city||o.city||'',postalCode:o.customer?.postalCode||o.postalCode||'',notes:o.customer?.notes||o.notes||''},destinationText:[o.customer?.address||o.address,o.customer?.city||o.city,o.customer?.postalCode||o.postalCode].filter(Boolean).join(', '),transportAmount:o.delivery,transportPaid:paidOrderStatus(o.status)||!!o.paidAt,items:o.items.map(x=>({ref:x.ref,title:x.title,qty:x.qty}))};const r=await rutaFVRequest(RUTAFV_DELIVERY_PATH,payload);o.transport.deliveryId=String(r.id||r.deliveryId||r.expeditionId||'');o.transport.status='creado_en_rutafv';o.transport.syncedAt=new Date().toISOString();operations.ensureLedgerForOrder(d,o);save(d);res.json(o)}catch(e){res.status(503).json({error:e.message})}});
