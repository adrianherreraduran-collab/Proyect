...
function decorateTransportQuote(q={},userId,items=[]){const amount=Math.max(0,Number(q.amount??q.total??0));const quoteId=String(q.id||q.quoteId||'');return {...q,amount,deliveryMode:'normal',express:false,estimatedDeliveryDate:q.estimatedDeliveryDate||q.deliveryDate||null,deliveryDateStatus:q.deliveryDateStatus||'pendiente_planificacion',_fvmSignature:transportQuoteSignature(userId,amount,quoteId,items)}}
function validTransportQuote(q,userId,items=[]){if(!q||q.deliveryMode==='express'||q.express===true)return false;const amount=Math.max(0,Number(q.amount??q.total??0));const quoteId=String(q.id||q.quoteId||'');const actual=Buffer.from(String(q._fvmSignature||''));const expected=Buffer.from(transportQuoteSignature(userId,amount,quoteId,items));return actual.length===expected.length&&crypto.timingSafeEqual(actual,expected)}
function moneyRound(value){return Math.round((Number(value)||0)*100)/100}
function paidOrderStatus(status){return new Set(['pagado','preparando','en_reparto','entregado']).has(String(status||'').toLowerCase())}
...
function buildOrder(d,user,items,customer={},options={}){const normalized=[];let subtotal=0;for(const item of items||[]){const p=d.products.find(x=>x.id===item.id&&x.published);if(!p)continue;const qty=Math.max(1,Math.min(99,Number(item.qty)||1)),unit=offerPrice(p);normalized.push({productId:p.id,title:p.title,ref:p.ref,unitPrice:unit,regularUnitPrice:Number(p.price||0),discountPct:Number(p.discountPct||0),qty,lineTotal:moneyRound(unit*qty),procurement:{provider:String(p.sourceProvider||providerFromUrl(p.sourceUrl)||''),sourceRef:String(p.sourceRef||''),sourceEan:String(p.sourceEan||''),sourceUrl:String(p.sourceUrl||''),sourcePrice:Number(p.sourcePrice)||0}});subtotal+=unit*qty}if(!normalized.length)return {error:'No hay productos válidos'};const requestedTransport=!!options.useRutaFV,q=options.quote||{};if(requestedTransport&&!validTransportQuote(q,user.id,items))return {error:options.paymentMethod==='stripe'?'Vuelve a calcular el transporte a tu obra antes de pagar.':'Vuelve a calcular el transporte a tu obra antes de confirmar el pedido.',status:409};const c=customer||{},destination={address:String(c.address||options.address||'').trim(),city:String(c.city||options.city||'').trim(),postalCode:String(c.postalCode||options.postalCode||'').trim(),notes:String(c.notes||options.notes||'').trim()},delivery=requestedTransport?Math.max(0,Number(q.amount??q.total??0)):0,total=moneyRound(subtotal+delivery),contactName=String(c.name||user.name||[user.firstName,user.lastName].filter(Boolean).join(' ')||'').trim();const order={id:options.id||id('ord'),number:options.number||'FVM-'+Date.now().toString().slice(-8),userId:user.id,items:normalized,subtotal:moneyRound(subtotal),delivery,transport:{provider:requestedTransport?'RutaFV':'',requested:requestedTransport,amount:delivery,quoteId:String(q.id||q.quoteId||''),deliveryMode:'normal',express:false,status:requestedTransport?'pendiente_crear_reparto':'sin_transporte',estimatedDeliveryDate:q.estimatedDeliveryDate||null,deliveryDateStatus:q.deliveryDateStatus||'pendiente_planificacion'},total,customer:{name:contactName,billingName:String(c.billingName||user.billingName||contactName).trim(),nifNie:String(c.nifNie||user.nifNie||'').trim(),billingAddress:String(c.billingAddress||user.billingAddress||'').trim(),billingCity:String(c.billingCity||user.billingCity||'').trim(),billingPostalCode:String(c.billingPostalCode||user.billingPostalCode||'').trim(),email:String(c.email||user.email||'').trim(),phone:String(c.phone||user.phone||options.phone||'').trim(),...destination},...destination,phone:String(c.phone||user.phone||options.phone||'').trim(),paymentMethod:options.paymentMethod||'transfer',status:'pendiente_pago',createdAt:new Date().toISOString()};return {order}}
