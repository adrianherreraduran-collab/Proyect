...
function read(){
  try{
    ...
    const changedAdmin=ensureAdmin(d);ensureCustomerData(d);ensureCatalogSettings(d);ensureWarehouseData(d);if(changedAdmin)save(d);else save(d);
    return d;
  }catch(e){
    const d=seed();ensureAdmin(d);ensureCustomerData(d);ensureCatalogSettings(d);ensureWarehouseData(d);save(d);return d;
  }
}
...
app.put('/api/admin/orders/:id',ordersManager,(req,res)=>{const d=read();const o=d.orders.find(x=>x.id===req.params.id);if(!o)return res.status(404).json({error:'Pedido no encontrado'});o.status=String(req.body.status||o.status);save(d);res.json(o)});
