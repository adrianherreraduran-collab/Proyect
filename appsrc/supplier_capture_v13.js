...
function registerProviderSourceRoutes(app,admin,deps){
  ...
  app.post('/api/admin/suppliers',admin,(req,res)=>{const d=ensureStores(deps.read());const name=compact(req.body.name);if(!name)return res.status(400).json({error:'El nombre del proveedor es obligatorio'});if(d.suppliers.some(s=>compact(s.name).toLowerCase()===name.toLowerCase()))return res.status(409).json({error:'Ya existe un proveedor con ese nombre'});const s={id:deps.id('sup'),name,website:compact(req.body.website).slice(0,300),defaultMargin:clamp(req.body.defaultMargin??40,0,300),notes:compact(req.body.notes).slice(0,1000),active:req.body.active!==false,createdAt:new Date().toISOString(),updatedAt:new Date().toISOString()};d.suppliers.unshift(s);deps.save(d);res.json(publicSupplier(s,d))});
  app.put('/api/admin/suppliers/:id',admin,(req,res)=>{const d=ensureStores(deps.read());const s=d.suppliers.find(x=>x.id===req.params.id);if(!s)return res.status(404).json({error:'Proveedor no encontrado'});const name=compact(req.body.name??s.name);if(!name)return res.status(400).json({error:'El nombre es obligatorio'});if(d.suppliers.some(x=>x.id!==s.id&&compact(x.name).toLowerCase()===name.toLowerCase()))return res.status(409).json({error:'Ya existe otro proveedor con ese nombre'});s.name=name;s.website=compact(req.body.website??s.website).slice(0,300);s.defaultMargin=clamp(req.body.defaultMargin??s.defaultMargin??40,0,300);s.notes=compact(req.body.notes??s.notes).slice(0,1000);if(req.body.active!=null)s.active=!!req.body.active;s.updatedAt=new Date().toISOString();deps.save(d);res.json(publicSupplier(s,d))});
