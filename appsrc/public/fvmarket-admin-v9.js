...
(()=>{
  ...
  function mountImporter(){
    ...
    card.dataset.v9='1';
    card.innerHTML=`<div class="bar"><div><h2 style="margin:0">Importar desde MiBricolaje · FVMarket IA</h2><p class="sub" style="margin:5px 0 0">Usa la ficha de origen solo para datos de aprovisionamiento y referencia visual. FVMarket crea su propia referencia, texto e imágenes.</p></div><span class="badge">Importador inteligente</span></div>
      <div class="notice"><b>Flujo:</b> referencia/URL → datos de origen → Ref. FVMarket propia → renderizar imágenes con IA → elegir imágenes → importar como borrador.</div>
      ...
      <div class="v9Help"><b>Imágenes:</b> las fotos del proveedor se muestran solo como referencia interna. Al pulsar <b>Renderizar imágenes IA</b>, FVMarket genera nuevas imágenes del producto con composición/fondo propios; eliges cuáles se guardan.</div>
  ...
  function genImgs(p,i){const a=p.images||[];if(!a.length)return `<button class="btn ghost" onclick="window.fvmV9Render(${i})">✨ Renderizar imágenes IA</button>${p.renderError?`<span class="v9ActionNote" style="color:#a32929">${esc(p.renderError)}</span>`:'<span class="v9ActionNote">Genera 3 opciones nuevas</span>'}`;return `<div class="v9ImgGrid">${a.map((im,j)=>`<div class="v9Img ${im.selected===false?'':'on'}" onclick="window.fvmV9ToggleImage(${i},${j})"><img src="${esc(im.url)}"><small>IA FVMarket ${j+1}</small></div>`).join('')}</div><button class="btn ghost" style="margin-top:6px" onclick="window.fvmV9Render(${i})">↻ Generar otras</button>`}
  ...
  window.fvmV9Render=async i=>{
    ...
    try{const r=await apiCall('/api/admin/mibricolaje/render-images',{method:'POST',body:JSON.stringify({title:p.title,sourceRef:p.sourceRef,sourceImages:p.sourceImages||[],count:3})});p.images=(r.images||[]).map(x=>({...x,selected:true}));if(!p.images.length)throw new Error('La IA no devolvió imágenes.');}
    catch(e){p.renderError=e.message||'No se pudieron generar las imágenes.'}finally{p.renderBusy=false;renderImporter()}
  };
  async function renderSelected(){const ids=V9.products.map((p,i)=>p.selected?i:-1).filter(i=>i>=0);const msg=$id('mbV9ImportMsg');if(!ids.length){msg.textContent='Selecciona al menos un producto.';return}for(let k=0;k<ids.length;k++){msg.textContent=`Renderizando ${k+1} de ${ids.length}…`;await window.fvmV9Render(ids[k])}msg.textContent='Imágenes IA preparadas. Revisa y desmarca las que no quieras.'}
  async function doImport(){
    ...
    const missing=chosen.filter(p=>!(p.images||[]).some(im=>im.selected!==false));if(missing.length){msg.textContent='Antes de importar, genera y selecciona al menos una imagen IA para cada producto.';return}
  ...
  window.loadProducts=async function(){
    ...
    const table=$id('products')?.closest('table');if(table){const h=table.querySelector('thead tr');if(h)h.innerHTML='<th>Ref. FVMarket</th><th>Ref. original</th><th>Proveedor</th><th>Producto</th><th>Categoría</th><th>Precio origen</th><th>Valor añadido</th><th>PVP</th><th>Estado</th><th>Acciones</th>'}
    const tbody=$id('products');if(!tbody)return;
    tbody.innerHTML=ps.map(p=>{const base=Number(p.sourcePrice||0),added=p.addedValue!=null?Number(p.addedValue):Math.max(0,Number(p.price||0)-base);return `<tr><td><b>${esc(p.ref||'—')}</b></td><td>${esc(p.sourceRef||'—')}</td><td>${esc(p.sourceProvider||'—')}</td><td>${esc(p.title||'')}</td><td>${esc(p.category||'')}</td><td>${eur(base)}</td><td>${eur(added)}</td><td><b>${eur(p.price)}</b></td><td><span class="badge">${p.published?'Publicado':'Borrador'}</span></td><td><div class="v9Actions"><button class="btn navy" onclick="openEdit('${p.id}')" title="Modificar ficha, precios, referencias e imágenes">Editar ficha</button><button class="btn ghost" onclick="quickAI('${p.id}')" title="Generar nuevas imágenes IA usando la referencia visual del producto">✨ IA imágenes</button><button class="btn ghost" onclick="toggle('${p.id}',${!p.published})" title="${p.published?'Retirar el producto de la tienda sin borrarlo':'Hacer visible el producto en la tienda'}">${p.published?'Ocultar':'Publicar'}</button><button class="btn danger" onclick="delp('${p.id}')" title="Eliminar definitivamente el producto">Eliminar</button></div></td></tr>`}).join('')||'<tr><td colspan="10" class="empty">No hay productos.</td></tr>';
  };
  window.quickAI=async function(id){
    ...
    try{const r=await apiCall('/api/admin/mibricolaje/render-images',{method:'POST',body:JSON.stringify({title:p.title,sourceRef:p.sourceRef,sourceImages:p.sourceImages,count:3})});const fresh=r.images||[];if(!fresh.length)throw new Error('La IA no devolvió imágenes.');const existing=Array.isArray(p.images)?p.images:[];const merged=[...fresh,...existing].slice(0,12);await apiCall('/api/admin/products/'+id,{method:'PUT',body:JSON.stringify({images:merged,image:merged[0]?.url||p.image||''})});await window.loadProducts();openEdit(id)}catch(e){alert(e.message||'No se pudieron generar imágenes IA.')}
  };
